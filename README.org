...
** Haskell
This implementation is the only one different conceptually, the others one when they find a *[* or *]* cycle throw the program until they find the matching parentheses, but in Haskell there is no way to loop unless using recursion (for what I know right now!), so I prefered to use a stack to save the position and jump back when they are needed.
Obviously I can implement some extra helper function to loop on the string and get the correct position, but be /laziness/ of haskell corrupted me... (this solution is also faster).
